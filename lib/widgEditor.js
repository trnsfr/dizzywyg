...
var widgStylesheet = "/stylesheets/manager/widg/widgContent.css";
...
widgToolbarItems.push("floatleft");
widgToolbarItems.push("floatright");
widgToolbarItems.push("sizedown");
widgToolbarItems.push("sizeup");
...
function widgInit()
{
	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off"))
	{
		...
		setTimeout("new widgEditor('" + theTextarea.id + "')", 500 * (1));
...
widgEditor.prototype.modifyFormSubmit = function()
{
  ...
  oldSubmit = Object.clone(theForm.onsubmit);
  ...
  else if (theUrl.match('/editor/'))
  {
   theForm.onsubmit = function()
   {
     self.updateWidgInput();
     new Ajax.Request(theUrl, {asynchronous:true, evalScripts:true, onComplete:function(request){window.opener.location.reload();}, parameters:Form.serialize(this)}); return false;
   }
  }
	else
	{
		...
		{
      self.updateWidgInput();
      new Ajax.Request(theUrl, {asynchronous:true, evalScripts:true, parameters:Form.serialize(this)}); return false;
		}
	}
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "floatright":
				this.addButton(this.theList.id + "ButtonFloatRight", "widgButtonFloatRight", "FloatRight", "floatright");

				break;
				
  		case "floatleft":
  			this.addButton(this.theList.id + "ButtonFloatLeft", "widgButtonFloatLeft", "FloatLeft", "floatleft");

  			break;
  			
  		case "sizedown":
  			this.addButton(this.theList.id + "ButtonSizeDown", "widgButtonSizeDown", "SizeDown", "sizedown");

  			break;
  			
			case "sizeup":
  			this.addButton(this.theList.id + "ButtonSizeUp", "widgButtonSizeUp", "SizeUp", "sizeup");

  			break;
				
			case "hyperlink":
				this.addButton(this.theList.id + "ButtonLink", "widgButtonLink", "Hyperlink", "link", true);
			...
			case "justifyleft":
				this.addButton(this.theList.id + "ButtonJustifyLeft", "widgButtonJustifyLeft", "Justify Left", "justifyleft");

				break;
				
			case "justifycenter":
				this.addButton(this.theList.id + "ButtonJustifyCenter", "widgButtonJustifyCenter", "Justify Center", "justifycenter");

				break;
				
			case "justifyright":
				this.addButton(this.theList.id + "ButtonJustifyRight", "widgButtonJustifyRight", "Justify Right", "justifyright");
...
widgToolbar.prototype.addButton = function(theID, theClass, theLabel, theAction, lbOn)
{
	...
	if (lbOn == true) 
	{
	  theLink.href = "/editor/links/new?&authenticity_token=" + AUTH_TOKEN;
    new lightbox(theLink);
  }
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "floatleft":
      float("left", theIframe, theToolbar, theWidgEditor, this);
			
			break;
						
		case "floatright":
      float("right", theIframe, theToolbar, theWidgEditor, this);
     
			break;
			
		case "sizedown":
		
		  theSelection = determineSelection(theIframe);
		  
		  if (theSelection == null) {return false};
		  
		  if (theSelection.focusNode.firstChild) {
		    resizeImage("down", theIframe, theSelection);
		  }
		  else
		  {
	      resizeText("down", theIframe, theSelection);
		  }
      ...
      theSelection = determineSelection(theIframe);
		  ...
		  if (theSelection.focusNode.firstChild) {
		    resizeImage("up", theIframe, theSelection);
		  }
		  else
		  {
	      resizeText("up", theIframe, theSelection);
		  }
...
function createLink(name, url) {
  var theToolbar = $$('.widgToolbar')[0].widgToolbarObject;
	var theWidgEditor = theToolbar.widgEditorObject;
	var theIframe = theWidgEditor.theIframe;
...
function float(direction, theIframe, theToolbar, theWidgEditor, element) {
  var theAction = direction.replace(/^./, function(match){return match.toUpperCase();});
  
  theSelection = determineSelection(theIframe);
  
  theImage = $A(theSelection.focusNode.childNodes).find(function(s) {
  ...
  if (theImage == null)
	...
	if (element.parentNode.className.classExists("on"))
	{
		theImage.setAttribute("style", "");
		theWidgEditor.theToolbar.setState("Float" + theAction, "off");
	}
	else
	{
    theImage.setAttribute("style", "float:" + direction + "; padding: 4px;");
    theWidgEditor.theToolbar.setState("Float" + theAction, "on");
  }
}


function resizeImage(direction, theIframe, theSelection) {
  
  theImage = $A(theSelection.focusNode.childNodes).find(function(s) {
    return s.tagName == 'IMG';
  })
    
  new_width = parseInt(theImage.width) + 10;
    
  if (direction == 'down')
    new_width = parseInt(theImage.width) - 10;
    
  theImage.setAttribute("width", new_width);
}


function resizeText(direction, theIframe, theSelection) {
  
  old_size = 5;
  
  if (theSelection.focusNode.parentNode.size)
    old_size = theSelection.focusNode.parentNode.size;
    
  new_size = (direction == 'down') ? (parseInt(old_size) - 1) : (parseInt(old_size) + 1);
  ...
  theIframe.contentWindow.document.execCommand("FontSize", false, new_size);
}


function determineSelection(theIframe){
  if (theIframe.contentWindow.document.selection)
	{
...
function widgToolbarCheckState(theWidgEditor, resubmit)
{
	...
	if (theParentNode.nodeName.toLowerCase() == "body")
	{
  	theImage = $A(theParentNode.childNodes).find(function(s) {
      return s.tagName == 'IMG';
    })
  
    if (theImage != null)
    {
  		if (theImage.getAttribute("style") == "float: right; padding: 4px;")
  		{
  			theWidgEditor.theToolbar.setState("FloatRight", "on");
  		}
  		else if (theImage.getAttribute("style") == "float: left; padding: 4px;")
  		{
  			theWidgEditor.theToolbar.setState("FloatLeft", "on");
  		}
  	}
	}
	...
	while (theParentNode.nodeName.toLowerCase() != "body")
	{
		switch (theParentNode.nodeName.toLowerCase())
		{
			...
			case "span":
				...
				else if (theParentNode.getAttribute("style") == "text-align: left;")
				{
					theWidgEditor.theToolbar.setState("JustifyLeft", "on");
				}
				else if (theParentNode.getAttribute("style") == "text-align: center;")
				{
					theWidgEditor.theToolbar.setState("JustifyCenter", "on");
				}
				else if (theParentNode.getAttribute("style") == "text-align: right;")
				{
					theWidgEditor.theToolbar.setState("JustifyRight", "on");
				}
